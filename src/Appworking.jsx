...
export default function AICompanyWebsite() {
  ...
  // Animated number for AifnnSol card
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0A2342] via-[#0E1E3F] to-black text-white antialiased">
      ...
      <nav className="max-w-7xl mx-auto px-6 py-6 flex items-center justify-between">
        <div className="flex items-center gap-3">
         <div> <img src="AifNN_darkbluebackground1.png" alt="logo" width={100} height={100}/></div>
          <div>

            <div className="text-xs text-gray-400">AI • ML • Automation • Digitization</div>
          </div>
        </div>
      ...
      <section id="services" className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="mt-10 grid md:grid-cols-3 gap-8">
          {[
            { title: "Custom Vision Systems", desc: "Object detection, segmentation, and tracking for industrial and defense." },
            { title: "Data Engineering & MLOps", desc: "Pipelines, monitoring, feature stores, and reproducible ML training." },
            { title: "Edge & Embedded AI", desc: "Deploy optimized AI models on edge devices with minimal latency." },
          ].map((s) => (
            <motion.div
              key={s.title}
              whileHover={{ scale: 1.05, boxShadow: "0px 0px 20px rgba(0, 128, 255, 0.5)" }}
      ...
      <motion.section
        whileHover={{ scale: 1.02, boxShadow: "0px 0px 30px rgba(0, 128, 255, 0.3)" }}
        className="min-h-screen bg-gradient-to-b from-[#0A2342] via-[#0E1E3F] to-black flex items-center justify-center transition"
      >
      ...
      {/* AifnnSol Animated Card */}
      <motion.div
        ref={cardRef}
        whileHover={{ scale: 1.05, boxShadow: "0px 0px 20px rgba(0, 128, 255, 0.5)" }}
        className="max-w-xs mx-auto mt-10 rounded-2xl bg-gradient-to-r from-[#0d1f3b] to-[#132d55] p-6 shadow-lg text-center transition"
      >
      ...
      <section id="cases" className="max-w-7xl mx-auto px-6 py-16 bg-gradient-to-r from-gray-950 to-gray-900 rounded-3xl">
        ...
        <div className="mt-10 grid md:grid-cols-3 gap-8">
          {[
            { title: "AI in Agriculture", desc: "Smart crop monitoring and predictive analytics improved yields by 30%." },
            { title: "Defense Surveillance", desc: "Deployed battlefield surveillance AI reducing human monitoring load by 60%." },
            { title: "Automotive Safety", desc: "Enhanced driver-assist with real-time vision AI for accident prevention." },
          ].map((c) => (
            <motion.div
              key={c.title}
              whileHover={{ scale: 1.05, boxShadow: "0px 0px 20px rgba(0, 128, 255, 0.5)" }}
